...
sleep_sec_measure = 60*5 # deep sleep in sec between measure 
...
def go_to_deepsleep(sec):
  ...
  led.on()
  mosfet_pin.off() # turn off mosfet to cut power 
...
def blynk_thread(a):
  ...
  def connect_handler():
    ...
    if (blynk.is_server_alive()) == True:
      ...
      # send notification for first failed, or first recovered only, based on status stored in RTC memory
      ...
      if vcell != None: # sensor OK
        ...
        if r.memory()[1] == sensor_failed: # we were in a sensor error state.
          ...
          x[1]= sensor_ok
          r.memory(x)
          print ('reset lipo failed error status ', r.memory())
        else:
          pass # status is OK and sensor read is OK

      else: # error lipo sensor
        ...
        if r.memory()[1] == sensor_ok: # we were in a sensor ok state.
          ...
          x[1]= sensor_failed
          r.memory(x)
          print ('reset lipo ok error status ', r.memory())
        else:
      ...
      if temp != []: # temp sensor OK
        oled.fill(0)
        blynk.virtual_write(vpin_haut, temp_haut)
        blynk.virtual_write(vpin_milieu, temp_milieu)      
        oled.text(("haut:   " + str(temp_haut)), 0, 0)
        oled.text(("milieu: " + str(temp_milieu)), 0, 10)
        sleep(3)
        oled.show()
        ...
        if r.memory()[0] == sensor_failed: # we were in a sensor error state.
          ...
          x[0]= sensor_ok
          r.memory(x)
          print ('temp error status ', r.memory()) 
        else:
          pass # status is OK and sensor read is OK

      else: # temp sensor failed
        oled.text("haut failed", 0, 0)
        oled.text("milieu failed", 0, 8)
...
# DS18B20 aka dallas
# vcc 3 to 5V
# ISSUE. when powering oled using a gpio, i2c detect dies bot see it, but wrirren to it is OK 
##############################################
def read_temp(gpio):
...
def end_app(a):
  ...
  print('wait for connect Blynk call back to read and update sensors') # wait for sensor value update into blynk server
...
try:
  ...
  oled.fill(0) # with black , 1 with white
  oled.text("Pabou ECS", 0, 2) # X,Y
  oled.show()
except Exception as e:
  print('Exception oled ' , str(e))
...
for i in range(len(mynet.net)):
  ...
  if wifi != None:
    ...
    oled.text('rssi: ' + (str(wifi.status('rssi'))), 0, 10)
    oled.text("sleep mnc: " + str(int(sleep_sec_measure/60)), 0, 20)
    oled.show()
    break
...
# will wait for blynk to run and call deep sleep or idel
